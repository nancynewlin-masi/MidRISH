...
# Does it need to be registered? Yes

template_filename = sys.argv[1]
sh_filename = sys.argv[2]
outlocation = sys.argv[3]
print("Applying template A -> mid space")
...
print(template_imagedata.shape)
positives = template_imagedata > 0
template_imagedata[template_imagedata <= 0] = 0 
print(template_imagedata.shape)
...
#    if harmonicorder[volume]<3:
	...
	sh_outarray[:,:,:,volume] = np.sqrt(template_imagedata[:,:,:,templateindex[volume]]) * sh_imagedata[:,:,:,volume]
